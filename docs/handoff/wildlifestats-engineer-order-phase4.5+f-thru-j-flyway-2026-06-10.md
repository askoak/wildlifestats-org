...
- **4.5+g — Apify client + extraction + smoke test** — SHIPPED. PR #33, merged `69887dc`. Code (apify_client.py, extract.py offline+llm, extraction-prompt.md, extraction-record-schema.json). Smoke: 47 real posts (31 FB + 16 IG, official actors), offline matcher 0, LLM extraction 6 baby-season records (NY+TN), audit clean (post_text_NOT_STORED), cost ≈ $0.30. (Gate was red from architect content #31/#32; engineer ran the authorized html-validate sweep, PRs #34 + #36, to unblock.)
